...
/*
 * 加载 JVM 的共享库（libjvm），并获取相关的 JVM 函数指针以用于启动 JVM。
 */
jboolean
LoadJavaVM(const char *jvmpath, InvocationFunctions *ifn)
{
    ...
    // 使用dlopen加载libjvm库，RTLD_NOW表示立即解析符号，RTLD_GLOBAL使得符号可以被后续加载的模块使用
    ...
    // 获取JNI_CreateJavaVM函数的地址，用于创建Java虚拟机实例
    ...
    // 获取JNI_GetDefaultJavaVMInitArgs函数的地址，用于获取默认的JVM初始化参数
    ...
    // 获取JNI_GetCreatedJavaVMs函数的地址，用于获取当前创建的JVM实例
